...
func (m *mockLocationService) UpdateLocation(ctx context.Context, req *pb.UpdateLocationRequest) (*pb.UpdateLocationResponse, error) {
	m.locations[req.UserId] = &pb.GetLocationResponse{Lat: req.Lat, Lng: req.Lng}
...
func TestUpdateLocation(t *testing.T) {
	...
	resp, err := service.UpdateLocation(context.Background(), &pb.UpdateLocationRequest{UserId: 1, Lat: 52.52, Lng: 13.405})
...
func TestGetLocation(t *testing.T) {
	service := setupMockService()
	service.locations[1] = &pb.GetLocationResponse{Lat: 52.52, Lng: 13.405}
	resp, err := service.GetLocation(context.Background(), &pb.GetLocationRequest{UserId: 1})
	assert.NoError(t, err)
	assert.Equal(t, 52.52, resp.Lat)
	assert.Equal(t, 13.405, resp.Lng)
}
...
func TestUpdateMultipleUsers(t *testing.T) {
	service := setupMockService()
	service.UpdateLocation(context.Background(), &pb.UpdateLocationRequest{UserId: 1, Lat: 40.7128, Lng: -74.0060})
	service.UpdateLocation(context.Background(), &pb.UpdateLocationRequest{UserId: 2, Lat: 34.0522, Lng: -118.2437})
	...
	assert.Equal(t, 40.7128, resp1.Lat)
	assert.Equal(t, -74.0060, resp1.Lng)
	assert.Equal(t, 34.0522, resp2.Lat)
	assert.Equal(t, -118.2437, resp2.Lng)
}
